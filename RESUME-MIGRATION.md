...
| 8a (NEW) | override #7 | Migration window: set old Firestore rules to `allow write: if false` so existing users get write-failures and prompt-update. Documented as a one-line README note + Supabase Studio step. |
